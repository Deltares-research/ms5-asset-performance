...
if __name__ == "__main__":
    ...
    for package in packages:
        ...
        moments = [m[0] for m in results["moment"] if len(m[0]) == 156]
        moments = np.asarray(moments).squeeze()
    ...
    df_data = pd.DataFrame(
        data=np.column_stack([idxs, Xs, ys, ms]),
        columns=["index"]+rv_names+[f"disp_{i+1}" for i in range(ys.shape[1])]+[f"moment_{i+1}" for i in range(ms.shape[1])]
    )
    cols_keep = [
    ...
    df_data = df_data.loc[:, ["index"]+cols_keep+[f"disp_{i+1}" for i in range(ys.shape[1])]+[f"moment_{i+1}" for i in range(ms.shape[1])]]

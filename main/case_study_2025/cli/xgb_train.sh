...
#      result=$(python "$SCRIPT_DIR/train/surrogate/xgb_train.py" --n-estimators "$est" --max-depth "$d" --lr "$lr" 2>&1 | tee /dev/tty)
      result=$(python -m main.case_study_2025.train.surrogate.xgb_train --n-estimators "$est" --max-depth "$d" --lr "$lr" 2>&1 | tee /dev/tty)
      result="${result}\n"

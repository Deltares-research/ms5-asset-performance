...
for lr_exp in "${lr_exps[@]}"; do
  for ep in "${epochs[@]}"; do
    ...
    if [ "$fullprofile" = "true" ]; then
      result=$(python -m main.case_study_2025.train.surrogate.mlp_moment_train --lr "$lr" --epochs "$ep" --full-profile 2>&1 | tee /dev/tty)
    else
      result=$(python -m main.case_study_2025.train.surrogate.mlp_moment_train --lr "$lr" --epochs "$ep" 2>&1 | tee /dev/tty)
    fi
    result="${result}\n"

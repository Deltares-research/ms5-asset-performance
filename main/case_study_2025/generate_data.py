...
if __name__ == "__main__":
    ...
    rv_klei = MvnRV(mus=np.array([30, 10]), stds=np.array([3, 1]), names=["Klei_soilphi", "Klei_soilcohesion"])
    rv_zand = MvnRV(mus=np.array([35]), stds=np.array([4]), names=["Zand_soilphi"])
    rv_wall = MvnRV(mus=np.array([45_000]), stds=np.array([5_000]), names=["Wall_SheetPilingElementEI"])
    state = GaussianState(rvs=[rv_klei, rv_zand, rv_wall])
    ...
    if config["n_locs"] == 1:
        draw_sample(state, geomodel, config, path=r"data/sample.json")
    else:
        draw_sample(state, geomodel, config, path=f"data/sample_{config['n_locs']}_{config['rv_pooling']}.json")

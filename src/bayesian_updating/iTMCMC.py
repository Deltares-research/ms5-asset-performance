...
def iTMCMC(Ns, Nb, l_class, T_nataf, max_it: int = 50):  
    ...
    parameter_values = np.zeros((max_it+1, Ns))
    cur_parameter_values = np.zeros(Ns)
    ...
    logL_j = np.zeros(Ns)
    for i in range(Ns):
        logL_j[i] = l_class.compute_log_likelihood_for_parameters(theta_j[i,:].flatten())
        cur_parameter_values[i] = l_class.parameter_value
    # print("cur_parameter_values: ", cur_parameter_values)
    parameter_values[0, :] = cur_parameter_values
    ...
    while q[j] < 1 and j < max_it:   # adaptively choose q
        ...
        theta_c  = theta_j
        logL_c   = logL_j
        cur_cur_parameter_value = cur_parameter_values
        Nadapt   = 1
        # cur_parameter_values = parameter_values[0, :]
        na, acpt = 0, 0
        for k in range(Ns+Nb):      
            ...
            logL_star  = l_class.compute_log_likelihood_for_parameters(theta_star.flatten())
            ...
            if rv <= ratio:
                ...
                logL_c[l]    = logL_star
                acpt        += 1
                cur_cur_parameter_value[l] = l_class.parameter_value
            
            if k > Nb:   # (Ref. 1 Modification 2: burn-in period)
                u_j[k-Nb,:]     = u_c[l,:]
                theta_j[k-Nb,:] = theta_c[l,:]
                logL_j[k-Nb]    = logL_c[l]
                cur_parameter_values[k-Nb] = cur_cur_parameter_value[l]
        ...
        parameter_values[j, :] = cur_parameter_values
    ...
    if j < max_it:
        q = q[:j+1]
        logS_j = logS_j[:j]
        parameter_values = parameter_values[:j+1, :]
    
    # evidence (normalization constant in Bayes' theorem)
    logcE = np.sum(logS_j)   # [Ref. 2 Eq. 17]
    
    return samplesU, samplesX, q, logcE, parameter_values
